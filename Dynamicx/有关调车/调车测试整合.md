...
```
在本地打开rqt（记得将bashrc里的ROS_IP改为：192.168.100.2）
打开controller_manager（插件没有可以用以下命令安装）
`sudo apt install ros-noetic-rqt* #待测试`

开trigger controllers(校准拨盘，校准结束后关闭)

开shooter controllers

去插件（plugins）那里拉一个Message publisher
然后选择结尾是shooter_controllers/command（应该是这个）的
然后参数的意思是：
```
mode: 0
wheel_speed: 0.0
...
###前置知识（会的跳过）：
串行接口简称串口，也成串行通信接口或串行通讯接口（COM？），数据一般是一位一位地顺序传送。
...
使用方式：
`sudo minicom -s`
然后选择第三个，回车，按a就能改串口类型，然后
在/dev后面加上usbReferee
`/dev/usbReferee`
然后回到原来界面，选择exit，如果出现界面说明有串口，如果有数据说明串口数据正常，如果没有说明没有读到数据。
也可以把usbReferee换成其他的串口来读其他的串口情况